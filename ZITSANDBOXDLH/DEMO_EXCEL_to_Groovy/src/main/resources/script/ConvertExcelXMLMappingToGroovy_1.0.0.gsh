...
/**
* ConvertExcelXMLMappingToGroovy
* This Groovy script reads an Excel mapping and creates a Groovy Script mapping template. A single attribute is also mapped to a node.
...
* - ConvertMap.PlainColumnSeparator = Plain column separator
*
...
*/
	...
	final String PLAIN_COLUMN_SEPARATOR = '\t'
...
*/
	...
	// Debug mode
	debugModeIn = getExchangeProperty(message, "ConvertMap.DebugMode", false)
	if (debugModeIn) {
	...
	createOnlyDebugIn = getExchangeProperty(message, "ConvertMap.CreateOnlyDebug", false)
	...
	createTargetDescriptionIn = getExchangeProperty(message, "ConvertMap.CreateTargetDescription", false)
	...
	createTargetXpathIn = getExchangeProperty(message, "ConvertMap.CreateTargetXpath", false)
	...
	createMapRuleIn = getExchangeProperty(message, "ConvertMap.CreateMapRule", false)
	if (createMapRuleIn) {
	...
	createNoConvertableFormulaIn = getExchangeProperty(message, "ConvertMap.CreateNoConvertableFormula", false)
	...
	createSegmentHierarchyIn = getExchangeProperty(message, "ConvertMap.CreateSegmentHierarchy", false)
	...
	createRootTargetNamespace1In = getExchangeProperty(message, "ConvertMap.CreateRootTargetNamespace1", false)
	...
	createNodesTargetNamespace1In = getExchangeProperty(message, "ConvertMap.CreateNodesTargetNamespace1", false)
	...
	targetNamespacePrefix1 = getExchangeProperty(message, "ConvertMap.TargetNamespacePrefix1", false)
	...
	// Target Namespace 1
	targetNamespace1 = getExchangeProperty(message, "ConvertMap.TargetNamespace1", false)
	if (!targetNamespace1) {
	...
	appendAllGroovyCostumFunctionsIn = getExchangeProperty(message, "ConvertMap.AppendAllGroovyCostumFunctions", false)
	...
	appendMappingExamplesIn = getExchangeProperty(message, "ConvertMap.AppendMappingExamples", false)
	...
	// Append plain column separator
	plainColumnSeparator = getExchangeProperty(message, "ConvertMap.PlainColumnSeparator", false)
	if (!plainColumnSeparator) {
		plainColumnSeparator = PLAIN_COLUMN_SEPARATOR
	}

/**
* Read csv body line values separated by tabulator to lists
*/

	// Initialize values
	int sourceFieldColumn
	int sourceStructureColumn
	int sourceFormulaColumn
	int targetFieldColumn
	int targetStructureColumn
	int isTargetSegmentColumn
	int targetDescriptionColumn
	int targetRuleColumn
	// Set default values
	sourceFieldColumn = null
	sourceStructureColumn = null
	sourceFormulaColumn = null
	targetFieldColumn = null
	targetStructureColumn = null
	isTargetSegmentColumn = null
	targetDescriptionColumn = null
	targetRuleColumn = null
	...
	if (message.getBodySize() > 0) {
		...
		body.eachLine { line, count ->
			...
			if (lineLength > 0) {
				int indexOfSeparator = line.indexOf(plainColumnSeparator)
				if (indexOfSeparator != -1) {
					// Make max columns
					line = line + (plainColumnSeparator * 10)
					// Split line
					columList = line.split(plainColumnSeparator, -1)
					if (count == 0) {
						// Read header lines
						for (int i = 0; i < columList.size(); i++) {
							String columnEntry = columList[i].replaceAll('\\s+','').toLowerCase()
							columList[i] = columnEntry
							// Set columns
							switch(columnEntry) {
								case 'sourcefield':
									sourceFieldColumn = i
								...
								case 'sourcestructure':
									sourceStructureColumn = i
									break
								case 'sourceformula':
									sourceFormulaColumn = i
									break
								case 'targetfield':
									targetFieldColumn = i
									break
								case 'targetstructure':
									targetStructureColumn = i
									break
								case 'istargetsegment':
									isTargetSegmentColumn = i
									break
								case 'targetdescription':
									targetDescriptionColumn = i
									break
								case 'mappingrule':
									targetRuleColumn = i
									break
						...
						// Check columns
						if ((targetFieldColumn == null || targetFieldColumn < 0) && (targetStructureColumn == null || targetStructureColumn < 0)) {
							throw Exception("Mandatory column 'Target Field' is missing.")
						}
						if (targetStructureColumn == null || targetStructureColumn < 0) {
							throw Exception("Mandatory column 'Target Structure' is missing.")
						}
					} else {
						l ++
						// Get target node
						...
						String targetField = ''
						if (targetStructureColumn || targetStructureColumn == 0) {
							targetStructure = columList[targetStructureColumn].replaceAll('\\s+','')
						}
						// Check 'Target Structure' column
						if (count > 1) { // No check on root-node
							if (!targetStructure) {
								throw Exception("Mandatory value for column 'Target Structure' is empty.")
							}
						}
						if (targetFieldColumn || targetFieldColumn == 0) {
							targetField = columList[targetFieldColumn].replaceAll('\\s+','')
						}
						// Check 'Target Field' column
						if (count > 1) { // No check on root-node
							if (!targetField && !targetStructure) {
								throw Exception("Mandatory value for column 'Target Field' is empty.")
							}
						}
						if (!targetStructure.startsWith(XPATH_SEPARATOR)) {
							targetNode = XPATH_SEPARATOR
						} else {
							targetNode = ''
						}
						targetNode += targetStructure
						if (!targetNode.endsWith(XPATH_SEPARATOR) && targetField) {
							targetNode += XPATH_SEPARATOR
						}
						targetNode += targetField
						targetNode = targetNode.replaceAll('\\\\', XPATH_SEPARATOR)
						// Remove all namespace prefix for target node
						targetNode = removeAllPrefix(targetNode)
						// Set targetNodes[]
						targetNodes[l] = targetNode
						// Set isTargetSegmentColumn[]
						String isTargetSegmentString = ''
						if (isTargetSegmentColumn && isTargetSegmentColumn >= 0) {
							isTargetSegmentString = columList[isTargetSegmentColumn].trim().toLowerCase()
							if (isTargetSegmentString && 'xyesjatrue'.indexOf(isTargetSegmentString) > -1) {
								isTargetSegments[l] = true
							} else {
								isTargetSegments[l] = false
							}
						}
						// Set targetDescription[]
						if (targetDescriptionColumn || targetDescriptionColumn == 0) {
							targetDescription[l] = columList[targetDescriptionColumn].trim()
						}
						// Set mapRules[]
						if (targetRuleColumn || targetRuleColumn == 0) {
							mapRules[l] = columList[targetRuleColumn].trim()
						}
						// Get source node
						String sourceStructure = ''
						String sourceField = ''
						String sourceFormula = ''
						if (sourceStructureColumn || sourceStructureColumn == 0) {
							sourceStructure = columList[sourceStructureColumn].replaceAll('\\s+','')
						}
						if (sourceFieldColumn || sourceFieldColumn == 0) {
							sourceField = columList[sourceFieldColumn].replaceAll('\\s+','')
						}
						if (sourceFormulaColumn || sourceFormulaColumn == 0) {
							sourceFormula = columList[sourceFormulaColumn]
						}
						// Set sourceNodes[]
						sourceNode = sourceFormula
						...
						// Creat some valid values from formula
						// TODO-check
						if (sourceNode.startsWith("TODO")) {
							sourceType = 'constant'
							lineActivation[l] = false
							message.setProperty('ConvertMap.LastLineSourceFormula','TODO')
						// Constant value
						...
						// SimpleTrimZeroLeft
						} else if (sourceNode.startsWith('trimZeroLeft(') && bracketCount == 1) {
						...
						} else if (sourceNode.startsWith('"setCurrentDate(') && bracketCount == 1) {
						...
						} else if (sourceNode.startsWith('"setDefaultAsCurrentDate(') && bracketCount == 1) {
						...
						// SimpleHeadString
						} else if (sourceNode.startsWith('headString(') && bracketCount == 1) {
						...
						// SimpleTailString
						} else if (sourceNode.startsWith('tailString(') && bracketCount == 1) {
						...
						} else if (sourceNode.startsWith('fillLeadingZeros(') && bracketCount == 1) {
						...
						} else if (sourceNode.startsWith('fillUpToLengthWithSpace(') && bracketCount == 1) {
						...
						} else if (sourceNode.startsWith('throwExceptionIfNoValue(') && bracketCount == 1) {
						...
						// SimpleGetMessageID
						} else if (sourceNode.startsWith('getMessageID(') && bracketCount == 1) {
							sourceNode = getSimpleMessageID(sourceNode)
							sourceType = 'formula'
							message.setProperty('ConvertMap.LastLineSourceFormula','SimpleGetMessageID')
						// SimpleSender
						} else if ('sender()'.equals(sourceNode)) {
							sourceNode = getSimpleSender(sourceNode)
							sourceType = 'formula'
							message.setProperty('ConvertMap.LastLineSourceFormula','SimpleSender')
						// SimpleReceiver
						} else if ('receiver()'.equals(sourceNode)) {
							sourceNode = getSimpleReceiver(sourceNode)
							sourceType = 'formula'
							message.setProperty('ConvertMap.LastLineSourceFormula','SimpleReceiver')
						// Get Header
						} else if (sourceNode.startsWith('getHeader(') && bracketCount == 1) {
							sourceType = 'formula'
							message.setProperty('ConvertMap.LastLineSourceFormula','getHeader')
						// Get Property
						} else if (sourceNode.startsWith('getExchangeProperty(') && bracketCount == 1) {
							sourceType = 'formula'
							message.setProperty('ConvertMap.LastLineSourceFormula','getExchangeProperty')
						} else {
							// Use sourceStructure and sourceNode
							if (sourceStructure && !sourceStructure.startsWith(XPATH_SEPARATOR)) {
								sourceNode = XPATH_SEPARATOR
							} else {
								sourceNode = ''
							}
							sourceNode += sourceStructure
							if (sourceNode && !sourceNode.endsWith(XPATH_SEPARATOR)) {
								sourceNode += XPATH_SEPARATOR
							}
							...
							if (!sourceNode) {
								...
								// Set direct node mapping
								sourceNode = sourceNode.substring(1, sourceNode.length()) + ".text()"
								sourceNode = sourceNode.replaceAll('/','.').replaceAll('\\\\', XPATH_SEPARATOR)
						...
						if (sourceNode && sourceNode != "''") {
							...
							sourceTypes[l] = 'constant'
						}
					}
				} else {
					if ('\t'.equals(plainColumnSeparator)) {
						throw Exception("Input body is not a valid mapping with configured column separator 'tabulator'.")
					} else {
						throw Exception("Input body is not a valid mapping with configured column separator '$plainColumnSeparator'.")
					}
				}
			}
		}
...
*/
		...
		if (createOnlyDebug == false) {
			...
			sb.append(LINE_BREAK)
			sb.append(tabs + '// TODO: Create sub segments' + LINE_BREAK)
			...
			for (int i = 0; i < targetNodes.size(); i++) {
				...
				// Append Variable
					...
					sb.append(tabs + "String " + variableName + " = " + sourceNodes[i] + LINE_BREAK)
				} else if (lineActivation[i] == false && !attributeName && !variableName) {
					sb.append(tabs + "String " + nodeName + " = '' ")
					if (createNoConvertableFormula == true) {
						sb.append("// " + sourceNodes[i])
					}
					sb.append(LINE_BREAK)
...
private String removeAllPrefix(String value) {
	...
	if(value.indexOf('http:') == -1 && value.indexOf('currentDate(') == -1 && value.indexOf('TransformDate(') == -1 && !(value.indexOf('concat(') == 0 || bracketCount == 1)) {
		if (value.indexOf(':') > -1) {
			...
			// Join values
			value = tmpList.join(nodeSeparator)
		}
	}
	return value
}
...
private def computeIsTargetSegment(def targetNodes, def isTargetSegments) {
	...
	if (targetNodes != null) {
		...
		for (int i = 0; i < targetNodes.size(); i++) {
			if (i == 0) {
				nodeBefore = targetNodes[i]
				isTargetSegments[i] = false
			} else {
				nodeNext = targetNodes[i]
				isTargetSegments[i] = false
		...
		isTargetSegments = isTargetSegments.reverse()
	}
	return isTargetSegments
}
...
*/
private String getEmptyConstant(String sourceNode) {
	sourceNode = "''"
	return sourceNode
}
...
private String getConstantValue(String sourceNode, int indexOfFormulaSeparator, int indexOfClosingParenthesis) {
	sourceNode = "'" + sourceNode.substring(indexOfFormulaSeparator + 1, indexOfClosingParenthesis) + "'"
	return sourceNode
}
...
private String getDirectNode(String sourceNode) {
	sourceNode = sourceNode.substring(1, sourceNode.length())
	sourceNode += '.text()'
	sourceNode = sourceNode.replaceAll('/', '.')
	// Set simple quotation marks "'" around node name, if needed
	sourceNode = markNode(sourceNode)
	return sourceNode
}
...
private String getSimpleConcat(String sourceNode) {
	...
	// Create formula
	sourceNode = tmpList[0] + ".concat('" + tmpList[2] + "').concat(" + tmpList[1] + ")"
	return sourceNode
}
...
private String getSimpleTransformDate(String sourceNode) {
	sourceNode = sourceNode.replaceAll(', iform=',',_').replaceAll(', oform=',',_').replaceAll(', ',',_')
	String tmpSourceNode = sourceNode.substring(14, sourceNode.length() - 1)
	// Split into 4 parts
	def tmpList = []
	...
	// Create formula
	sourceNode = "transformDate(timeZone, " + tmpList[0] + '.text(), "' + tmpList[1] + '", "' + tmpList[2] + '")'
	return sourceNode
}
...
private String getSimpleGetDateAfterDays(String sourceNode) {
	sourceNode = sourceNode.replaceAll('/', '.').replaceAll('\\(\\.','(').replaceAll(', const\\(value=',',_').replaceAll(', ',',_').replaceAll('\\)','')
	String tmpSourceNode = sourceNode.substring(17, sourceNode.length())
	// Split into 2 parts
	def tmpList = []
	tmpList = tmpSourceNode.split(',_', -1)
	for (int i = 0; i < tmpList.size(); i++) {
	...
	// Create formula
	sourceNode = "getDateAfterDays(timeZone, " + tmpList[0] + '.text(), "yyyyMMdd", ' + tmpList[1] + ', "yyyyMMdd")'
	return sourceNode
}
...
private String getSimpleGetDateFormat(String sourceNode) {
	sourceNode = sourceNode.replaceAll('/', '.').replaceAll('\\(\\.','(')
	sourceNode = sourceNode.substring(14, sourceNode.length() - 1)
	sourceNode = markNode(sourceNode)
	sourceNode = 'getDateFormat(' + sourceNode + '.text())'
	return sourceNode
}
...
private String getSimpleFormatValueByZero(String sourceNode) {
	sourceNode = sourceNode.replaceAll('/', '.').replaceAll('\\(\\.','(').replaceAll(', const\\(value=',',_').replaceAll(', ',',_').replaceAll('\\)','')
	String tmpSourceNode = sourceNode.substring(18, sourceNode.length())
	// Split into 5 parts
	def tmpList = []
	tmpList = tmpSourceNode.split(',_', -1)
	for (int i = 0; i < tmpList.size(); i++) {
	...
	// Create formula
	sourceNode = "formatValueByZero(" + tmpList[0] + ".text(), " + tmpList[1] + ", '" + tmpList[2] + "', " + tmpList[3] + ", '" + tmpList[4] + "')"
	return sourceNode
}
...
private String getSimpleMapWithDefault(String sourceNode) {
	sourceNode = sourceNode.replaceAll('/', '.').replaceAll('\\(\\.','(').replaceAll(', result\\)',")").replaceAll(', default_value=',", ")
	String tmpSourceNode = sourceNode.substring(15, sourceNode.length() - 1)
	// Split into 2 parts
	def tmpList = []
	tmpList = tmpSourceNode.split(', ', -1)
	for (int i = 0; i < tmpList.size(); i++) {
	...
	// Create formula
	sourceNode = "mapWithDefault(" + tmpList[0] + ".text(), '" + tmpList[1] + "')"
	if (!tmpList[1]) {
		sourceNode += " // TODO: Please check if mapWithDefault() with an empty value is really needed here."
	}
	return sourceNode
}
...
private String getSimpleFixedValues(String sourceNode) {
	sourceNode = sourceNode.replaceAll('/', '.').replaceAll('\\(\\.','(').replaceAll(', table=FixedValues\\{',',_').replaceAll('\\}','').replaceAll(', vmstrategy=',',_').replaceAll(', vmdefault=',',_')
	String tmpSourceNode = sourceNode.substring(10, sourceNode.length() - 1)
	// Split into 4 parts
	def tmpList = []
	tmpList = tmpSourceNode.split(',_', -1)
	for (int i = 0; i < tmpList.size(); i++) {
	...
	// Create formula
	sourceNode = "fixedValues(" + tmpList[0] + ".text(), " + tmpList[1] + ", '" + tmpList[2] + "', '" + tmpList[3] + "')"
	return sourceNode
}
...
private String getSimpleValueMap(String sourceNode) {
	sourceNode = sourceNode.replaceAll('/', '.').replaceAll('\\(\\.','(').replaceAll('\\}','').replaceAll(', srcns=',',_')
	...
	String tmpSourceNode = sourceNode.substring(9, sourceNode.length() - 1)
	// Split into 8 parts
	def tmpList = []
	tmpList = tmpSourceNode.split(',_', -1)
	for (int i = 0; i < tmpList.size(); i++) {
	...
	// Create formula
	sourceNode = "valueMap(valueMapApi, '" + tmpList[2] + "', '" + tmpList[3] + "', " + tmpList[0] + ".text(), '" + tmpList[4] + "', '" + tmpList[5] + "', '" + tmpList[6] + "', '" + tmpList[7] + "')"
	return sourceNode
}
...
private String getSimpleFormatNumber(String sourceNode) {
	sourceNode = sourceNode.replaceAll('/', '.').replaceAll('\\(\\.','(').replaceAll('\\}','').replaceAll(', nformat=',',_').replaceAll(', separator=',',_')
	String tmpSourceNode = sourceNode.substring(13, sourceNode.length() - 1)
	// Split into 3 parts
	def tmpList = []
	tmpList = tmpSourceNode.split(',_', -1)
	for (int i = 0; i < tmpList.size(); i++) {
	...
	// Create formula
	sourceNode = "formatNumber(" + tmpList[0] + ".text(), '" + tmpList[1] + "', '" + tmpList[2] + "')"
	return sourceNode
}
...
private String getSimpleTrim(String sourceNode) {
	sourceNode = sourceNode.replaceAll('/', '.').replaceAll('\\(\\.','(')
	sourceNode = sourceNode.substring(5, sourceNode.length() - 1)
	sourceNode = markNode(sourceNode)
	sourceNode = sourceNode + '.text().trim()'
	return sourceNode
}
...
private String getSimpleTrimZeroLeft(String sourceNode) {
	int startIndex
	if (sourceNode.startsWith('removeLeadingZeros(')) {
		startIndex = 19
	...
	sourceNode = sourceNode.replaceAll('/', '.').replaceAll('\\(\\.','(')	
	sourceNode = sourceNode.substring(startIndex, sourceNode.length() - 1)
	sourceNode = markNode(sourceNode)
	sourceNode = methodName + '(' + sourceNode + '.text())'
	return sourceNode
}
...
private String getSimpleTrimRight(String sourceNode) {
	sourceNode = sourceNode.replaceAll('/', '.').replaceAll('\\(\\.','(')
	sourceNode = sourceNode.substring(10, sourceNode.length() - 1)
	sourceNode = markNode(sourceNode)
	sourceNode = 'trimRight(' + sourceNode + '.text())'
	return sourceNode
}
...
private String getSimpleSetCurrentDate(String sourceNode) {
	sourceNode = sourceNode.replaceAll('set', '').replaceAll('oform=', '')
	String dateFormat = ''
	if (sourceNode.indexOf(',') > -1) {
		dateFormat = sourceNode.substring(12, sourceNode.indexOf(','))
	} else {
		dateFormat = sourceNode.substring(13, sourceNode.length()-2)
		dateFormat = dateFormat.replaceAll('\"', '')
	}
	sourceNode = 'setCurrentDate(timeZone, "' + dateFormat + '")'
	return sourceNode
}
...
private String getSimpleSetDecimalSeparatorPoint(String sourceNode) {
	sourceNode = sourceNode.replaceAll('/', '.').replaceAll('\\(\\.','(')
	sourceNode = sourceNode.substring(25, sourceNode.length() - 1)
	sourceNode = markNode(sourceNode)
	sourceNode = 'setDecimalSeparatorPoint(' + sourceNode + '.text())'
}
...
private String getSimpleSetDefaultAsCurrentDate(String sourceNode) {
	sourceNode = sourceNode.replaceAll('/', '.').replaceAll('\\(\\.','(').replaceAll('const\\(value=', '').replaceAll('\\)', '')
	if (sourceNode.startsWith('"')) {
		sourceNode = sourceNode.substring(25, sourceNode.length()-1)
	} else {
		sourceNode = sourceNode.substring(24, sourceNode.length())
	}
	// Split into 2 parts
	def tmpList = []
	tmpList = sourceNode.split(', ', -1)
	for (int i = 0; i < tmpList.size(); i++) {
		String tmpValue = tmpList[i]
	...
	sourceNode = 'setDefaultAsCurrentDate(timeZone, ' + tmpList[0] + '.text(), "' + tmpList[1] + '")'
}
...
private String getSimpleStripSpaces(String sourceNode) {
	sourceNode = sourceNode.replaceAll('/', '.').replaceAll('\\(\\.','(')
	sourceNode = sourceNode.substring(12, sourceNode.length() - 1)
	sourceNode = markNode(sourceNode)
	sourceNode = 'stripSpaces(' + sourceNode + '.text())'
	return sourceNode
}
...
private String getSimpleToLowerCase(String sourceNode) {
	...
	sourceNode = sourceNode.substring(12, sourceNode.length() - 1)
	sourceNode = markNode(sourceNode)
	sourceNode = sourceNode + '.text().toLowerCase()'
	return sourceNode
}
...
private String getSimpleToUpperCase(String sourceNode) {
	...
	sourceNode = sourceNode.substring(12, sourceNode.length() - 1)
	sourceNode = markNode(sourceNode)
	sourceNode = sourceNode + '.text().toUpperCase()'
	return sourceNode
}
...
private String getSimpleCopyValue(String sourceNode) {
	sourceNode = sourceNode.replaceAll('/', '.').replaceAll('\\(\\.','(')
	sourceNode = sourceNode.substring(10, sourceNode.indexOf(','))
	sourceNode = markNode(sourceNode)
	sourceNode += '.text()'
	return sourceNode
}
...
private String getSimpleMinusFromBeginToEnd(String sourceNode) {
	sourceNode = sourceNode.replaceAll('/', '.').replaceAll('\\(\\.','(')
	sourceNode = sourceNode.substring(20, sourceNode.length() - 1)
	sourceNode = markNode(sourceNode)
	sourceNode = 'minusFromBeginToEnd(' + sourceNode + '.text())'
	return sourceNode
}
...
private String getSimpleMinusFromEndToBegin(String sourceNode) {
	...
	sourceNode = sourceNode.substring(20, sourceNode.length() - 1)
	sourceNode = markNode(sourceNode)
	sourceNode = 'minusFromEndToBegin(' + sourceNode + '.text())'
	return sourceNode
}
...
private String getSimpleDynamicSubstring(String sourceNode) {
	if (sourceNode.startsWith('substring(')) {
		startIndex = 10
		sourceNode = sourceNode.replaceAll('start=', '').replaceAll('count=', '')
	} else {
		startIndex = 17
	}
	sourceNode = sourceNode.replaceAll('/', '.').replaceAll('\\(\\.','(')
	...
	sourceNode = 'dynamicSubstring(' + tmpList[0] + '.text(), ' + tmpList[1] + ', ' + tmpList[2] + ')'
	return sourceNode
}
...
private String getSimpleReplaceString(sourceNode) {
	sourceNode = sourceNode.replaceAll('/', '.').replaceAll('\\(\\.','(').replaceAll('const\\(value=', '').replaceAll('\\)', '')
	sourceNode = sourceNode.substring(14, sourceNode.length())
	// Split into 3 parts
	def tmpList = []
	tmpList = sourceNode.split(', ', -1)
	for (int i = 0; i < tmpList.size(); i++) {
	...
	sourceNode = tmpList[0] + ".text().replaceAll('" + tmpList[1] + "', '" + tmpList[2] + "')"
	return sourceNode
}
...
private String getSimpleCount(String sourceNode) {
	sourceNode = sourceNode.replaceAll('/', '.').replaceAll('\\(\\.','(')
	sourceNode = sourceNode.substring(6, sourceNode.length() - 9)
	// Need to use size() instead of count()
	sourceNode = sourceNode + '.size()'
	return sourceNode
}
...
private String getSimpleToNumber(String sourceNode) {
	sourceNode = sourceNode.replaceAll('/', '.').replaceAll('\\(\\.','(')
	sourceNode = sourceNode.substring(9, sourceNode.length() - 1)
	sourceNode = markNode(sourceNode)
	sourceNode = 'toNumber(' + sourceNode + '.text())'
	return sourceNode
}
...
private String getSimpleRemoveAlgebraicSign(String sourceNode) {
	...
	sourceNode = sourceNode.substring(20, sourceNode.length() - 1)
	sourceNode = markNode(sourceNode)
	sourceNode = 'removeAlgebraicSign(' + sourceNode + '.text())'
	return sourceNode
}
...
private String getSimpleRemoveAlgebraicSignPlus(String sourceNode) {
	sourceNode = sourceNode.replaceAll('/', '.').replaceAll('\\(\\.','(')
	sourceNode = sourceNode.substring(24, sourceNode.length() - 1)
	sourceNode = markNode(sourceNode)
	sourceNode = 'removeAlgebraicSignPlus(' + sourceNode + '.text())'
	return sourceNode
}
...
private String getSimpleRemoveAllCharacters(String sourceNode) {
	...
	sourceNode = sourceNode.substring(20, sourceNode.length() - 1)
	sourceNode = markNode(sourceNode)
	sourceNode = 'removeAllCharacters(' + sourceNode + '.text())'
	return sourceNode
}
...
private String getSimpleRemoveAllSpaces(String sourceNode) {
	sourceNode = sourceNode.replaceAll('/', '.').replaceAll('\\(\\.','(')
	sourceNode = sourceNode.substring(16, sourceNode.length() - 1)
	sourceNode = markNode(sourceNode)
	sourceNode = 'removeAllSpaces(' + sourceNode + '.text())'
	return sourceNode
}
...
private String getSimpleRemoveAllSpecialCharacters(String sourceNode) {
	sourceNode = sourceNode.replaceAll('/', '.').replaceAll('\\(\\.','(')
	sourceNode = sourceNode.substring(27, sourceNode.length() - 1)
	sourceNode = markNode(sourceNode)
	sourceNode = 'removeAllSpecialCharacters(' + sourceNode + '.text())'
	return sourceNode
}
...
private String getSimpleGetOnlyNumbers(String sourceNode) {
	sourceNode = sourceNode.replaceAll('/', '.').replaceAll('\\(\\.','(')
	sourceNode = sourceNode.substring(15, sourceNode.length() - 1)
	sourceNode = markNode(sourceNode)
	sourceNode = 'getOnlyNumbers(' + sourceNode + '.text())'
	return sourceNode
}
...
private String getSimpleHeadString(String sourceNode) {
	sourceNode = sourceNode.replaceAll('/', '.').replaceAll('\\(\\.','(')
	sourceNode = sourceNode.substring(11, sourceNode.length() - 1)	
	// Split into 2 parts
	def tmpList = []
	tmpList = sourceNode.split(', ', -1)
	for (int i = 0; i < tmpList.size(); i++) {
	...
	sourceNode = 'headString(' + tmpList[0] + '.text(), ' + tmpList[1] + ')'
	return sourceNode
}
...
private String getSimpleTailString(String sourceNode) {
	...
	sourceNode = 'tailString(' + tmpList[0] + '.text(), ' + tmpList[1] + ')'
	return sourceNode
}
...
private String getSimpleFillLeadingZeros(sourceNode) {
	sourceNode = sourceNode.replaceAll('/', '.').replaceAll('\\(\\.','(')
	sourceNode = sourceNode.substring(17, sourceNode.length() - 1)	
	// Split into 2 parts
	def tmpList = []
	tmpList = sourceNode.split(', ', -1)
	for (int i = 0; i < tmpList.size(); i++) {
	...
	sourceNode = 'fillLeadingZeros(' + tmpList[0] + '.text(), ' + tmpList[1] + ')'
	return sourceNode
}
...
private String getSimpleFillUpToLengthWithSpace(sourceNode) {
	...
	sourceNode = 'fillUpToLengthWithSpace(' + tmpList[0] + '.text(), ' + tmpList[1] + ')'
	return sourceNode
}
...
private String getSimpleRemoveCarriageReturnLineFeed(sourceNode) {
	sourceNode = sourceNode.replaceAll('/', '.').replaceAll('\\(\\.','(')
	sourceNode = sourceNode.substring(29, sourceNode.length() - 1)
	sourceNode = markNode(sourceNode)
	sourceNode = 'removeCarriageReturnLineFeed(' + sourceNode + '.text())'
	return sourceNode
}
...
private String getSimpleRemoveDecimalIfZero(sourceNode) {
	...
	sourceNode = sourceNode.substring(20, sourceNode.length() - 1)
	sourceNode = markNode(sourceNode)
	sourceNode = 'removeDecimalIfZero(' + sourceNode + '.text())'
	return sourceNode
}
...
private String getSimpleReplaceSpecialCharacters(sourceNode) {
	...
	sourceNode = sourceNode.substring(25, sourceNode.length() - 1)
	sourceNode = markNode(sourceNode)
	sourceNode = 'replaceSpecialCharacters(' + sourceNode + '.text())'
	return sourceNode
}
...
private String getSimpleReplaceUmlauts(sourceNode) {
	...
	sourceNode = sourceNode.substring(15, sourceNode.length() - 1)
	sourceNode = markNode(sourceNode)
	sourceNode = 'replaceUmlauts(' + sourceNode + '.text())'
	return sourceNode
}
...
private String getSimpleThrowExceptionIfNoValue(sourceNode) {
	...
	sourceNode = "throwExceptionIfNoValue(" + tmpList[0] + ".text(), '" + tmpList[1] + "')"
	return sourceNode
}
...
* @return sourceNode Return source node.
*/
private String getSimpleMessageID(String sourceNode) {
	sourceNode = "message.getProperty('SAP_MessageProcessingLogID')"
	return sourceNode
}
...
* @return sourceNode Return source node.
*/
private String getSimpleSender(String sourceNode) {
	sourceNode = "message.getHeader('SAP_Sender', String)"
	return sourceNode
}
...
* @return sourceNode Return source node.
*/
private String getSimpleReceiver(String sourceNode) {
	sourceNode = "message.getHeader('SAP_Receiver', String)"
	return sourceNode
}
...
private String getComplexFormula(String sourceNode) {
	sourceNode = 'TODO: ' + sourceNode
	// Change '/' to '.' for easier use nodes in Groovy
	if(sourceNode.indexOf('http:') == -1) {
		sourceNode = sourceNode.replaceAll('/', '.').replaceAll('\\(\\.','(').replaceAll(', \\.',', ')
	}
	return sourceNode
}
...
private def appendMethodFixedValues(def sb, boolean methodFixedValues, boolean appendAllGroovyCostumFunctions, String lineBreak) {
	if (sb && methodFixedValues == true || sb && appendAllGroovyCostumFunctions == true) {
		...
		sb.append("	String result = ''" + lineBreak)
		sb.append("	behaviour = behaviour.toLowerCase().replaceAll(' ','')" + lineBreak)
		...
		sb.append("	} else if (!('defaultvalue'.equals(behaviour) || 'usekey'.equals(behaviour) || 'throwexception'.equals(behaviour))) {" + lineBreak)
		...
		sb.append('		switch(behaviour) {' + lineBreak)
...
private def appendMethodGetMondayOfWeek(def sb, boolean methodGetMondayOfWeek, boolean appendAllGroovyCostumFunctions, String lineBreak) {
	if (sb && methodGetMondayOfWeek == true || sb && appendAllGroovyCostumFunctions == true) {
		...
		sb.append('			}' + lineBreak)
		sb.append('			year = yearInt.toString()' + lineBreak)
		sb.append('		} catch (Exception ex) {' + lineBreak)
		...
		sb.append('			week = weekInt.abs().toString()' + lineBreak)
		sb.append('		} catch (Exception ex) {' + lineBreak)
		...
		sb.append('			weekOfYear = year + week' + lineBreak)
...
private def appendMethodSetFileName(def sb, boolean methodSetFileName, boolean appendAllGroovyCostumFunctions, String lineBreak) {
	if (sb && methodSetFileName == true || sb && appendAllGroovyCostumFunctions == true) {
		...
		sb.append('		if (!nameSeparator) {' + lineBreak)
		sb.append('			nameSeparator = NAME_SEPARATOR' + lineBreak)
		sb.append('		}' + lineBreak)
		sb.append('		// Add EDI message release' + lineBreak)
		sb.append('		if (messageVersion) {' + lineBreak)
		sb.append('			fileName += nameSeparator + messageVersion' + lineBreak)
		sb.append('		}' + lineBreak)
		sb.append('		// Add EDI message release' + lineBreak)
		sb.append('		if (messageRelease) {' + lineBreak)
		sb.append('			fileName += nameSeparator + messageRelease' + lineBreak)
		sb.append('		}' + lineBreak)
		sb.append('		// Add time stamp' + lineBreak)
		sb.append('		if (addTimeStamp) {' + lineBreak)
		sb.append('			try {' + lineBreak)
		sb.append('				if (!dateFormat) {' + lineBreak)
		sb.append('					dateFormat = DATE_FORMAT' + lineBreak)
		sb.append('				}' + lineBreak)
		sb.append('				// Set simple date format' + lineBreak)
		sb.append('				SimpleDateFormat sdf = new SimpleDateFormat(dateFormat)' + lineBreak)
		sb.append('				sdf.setTimeZone(timeZone)' + lineBreak)
		...
		sb.append('			}' + lineBreak)
		sb.append('			fileName += nameSeparator + timeStamp' + lineBreak)
		sb.append('		}' + lineBreak)
		sb.append('		// Add document number' + lineBreak)
		sb.append('		if (documentNumber) {' + lineBreak)
		sb.append('			fileName += nameSeparator + documentNumber' + lineBreak)
		sb.append('		}' + lineBreak)
		...
		sb.append('			}' + lineBreak)
		sb.append('			fileName += nameSeparator + messageId' + lineBreak)
		sb.append('		}' + lineBreak)
		...
		sb.append('			}' + lineBreak)
		sb.append('			fileName += nameSeparator + correlationId' + lineBreak)
		sb.append('		}' + lineBreak)
