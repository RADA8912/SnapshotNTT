...
def Message processData(Message message) {
    ...
    builder.'ord:SalesOrderRequest' {
        ...
        'SalesOrder' {
            ...
            'ExternalDocumentID' String.valueOf(input.purchaseProcessId)      
            ...
            'Payment' {
                ...
                'OnlinePayment' {
                    ...
                    'Amount' (input.payments.onlinePayment.amount, currencyCode: input.currency)
                }
                'InstallPlan' {
                    'TotalAmount' input.payments.installmentPlan.totalAmount,currencyCode: input.currency
                ...
                'Depositor' {
                    ...
                    'HouseNumber' input.payments.bankDetails.depositor.houseNumber
            ...
            'Partner' {
                ...
                'Address' {
                    'PhysicalAddress' {
                        ...
                        'HouseNumber' input.customer.address.houseNumber
            ...
            items.each { item ->
                'Item' {
                    ...
                    'GrossPrice' (item.price.gross, currencyCode: input.currency)                    
                    'DiscountArticle' (item.price.articleDiscount, currencyCode: input.currency)
                    'DiscountPosition' (item.price.positionDiscount, currencyCode: input.currency)
                    'VatRate' item.vatRate                    
                }
            }
            'Shipping' {
                'ShippingCharge' (input.shippingCharge.gross, currencyCode: input.currency)
                'CarrierName' input.shipment.carrier
                'CarrierService' input.shipment.service
            }          
            'Attachment' {
                'PdfFile' input.attachments.pdfFile
                'Url' input.attachments.url            
            }
        }
    }
