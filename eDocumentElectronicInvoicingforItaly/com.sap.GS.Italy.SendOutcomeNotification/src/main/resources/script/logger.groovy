...
def Message log_010(Message message) {
    ...
    try {
        ...
        processHeadersAndProperties(    log_id+'_REQ_info',           message);
        processBody(                    log_id+'_REQ_payload',   message);
        processAttachmentBody(          log_id+'_REQ_',   message);
...
def Message log_020(Message message) {
    ...
    try {
        increaseInternalHeaders(message);
        increaseInternalProperties(message);
        processHeadersAndProperties(    log_id+'_RSP_info',           message);
        processBody(                    log_id+'_RSP_payload',   message);
        processAttachmentBody(          log_id+'_RSP_',   message);
...
def Message log_030(Message message) {
    ...
    try {
        ...
        processAttachmentBody(          IFLOW_NAME+'_ERROR_',   message);
    } catch (Exception ex) {
        log.error("processData error",ex);
    }
    return message;
