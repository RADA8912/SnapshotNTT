...
@Field String IFLOW_NAME = 'Italy Send Invoice'
...
def Message processData_ERR(Message message) {
    Logger log = LoggerFactory.getLogger(this.getClass());
    
    try {
        processData( IFLOW_NAME+"_ERR", message);
    } catch (Exception ex) {
        log.error("processData error",ex);
    }
    return message;
...
def Message log_010(Message message) {
    ...
    try {
        ...
        processHeadersAndProperties(    log_id+'_Sign_REQ_info',           message);
        processBody(                    log_id+'_Sign_REQ_payload',   message);
        processAttachmentBody(          log_id+'_Sign_REQ_',   message);
...
def Message log_020(Message message) {
    ...
    try {
        increaseInternalHeaders(message);
        increaseInternalProperties(message);
        processHeadersAndProperties(    log_id+'_Sign_RSP_info',           message);
        processBody(                    log_id+'_Sign_RSP_payload',   message);
        processAttachmentBody(          log_id+'_Sign_RSP_',   message);
...
def Message log_030(Message message) {
    ...
    try {
        increaseInternalHeaders(message);
        increaseInternalProperties(message);
        processHeadersAndProperties(    log_id+"_SDIcall_REQ_info",           message);
        processBody(                    log_id+"_SDIcall_REQ_payload",   message);
        processAttachmentBody(          log_id+'_SDIcall_REQ_',   message);
    } catch (Exception ex) {
        log.error("processData error",ex);
    }
    return message;
...
def Message log_040(Message message) {
    ...
    try {
        increaseInternalHeaders(message);
        increaseInternalProperties(message);
        processHeadersAndProperties(    log_id+"_SDIcall_RSP_info",           message);
        processBody(                    log_id+"_SDIcall_RSP_payload",   message);
        processAttachmentBody(          log_id+'_SDIcall_RSP__',   message);
    } catch (Exception ex) {
        log.error("processData error",ex);
    }
    return message;
...
public byte[] readInputStream(InputStream inputStream) {
    ...
    try {
        ...
        log.error("readInputStream exception" + ex);
    }
...
def boolean isLoggingEnabled( Message message ) {
  ...
  String doLog = props.get( "loggingEnabled" );
...
def Message logByOperation( Message message )
{
	...
	try {
        ...
        def logE = message.getProperty("loggingEnabled");
        					
		if ( logE == "YES" ) {
			
			def props_0  = message.getProperties();
			def op     = getMapValue( props_0, "currentOperation" );
			op = op ? op : '';

			
			if ( op != '' ) {
				message.setProperty( "currentOperation", '' ); // clear operation name after logging
			} else {
