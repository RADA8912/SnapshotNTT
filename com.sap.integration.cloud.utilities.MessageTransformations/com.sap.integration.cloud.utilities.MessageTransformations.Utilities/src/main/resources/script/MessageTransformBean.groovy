...
def Message processData(Message message)
{
    ...
    switch(conversionType) {
        ...
        case "SimpleXML2Plain":
            ...
            InputStream iSBody = new ByteArrayInputStream(body.getBytes('UTF-8'));
            Node root = new XmlParser().parse(iSBody);
...
class StructurePlain2XML extends Transformation {
    ...
    void handleFieldNames(List<String> headerLines) {
        if (configStruct.processFieldNames == "fromConfiguration") {
            ...
            throw new Exception("Error! xml.processFieldNames = notAvailable is not yet supported for StructurePlain2XML!");
            //notAvailable - dummy names like 'Column1', 'Column2';
        }
    }
    ...
    public String buildPayload(String[] lines) {
        ...
        HashMap<String, Integer> recordsetMap = [:];
            ...
            for (int indexLine = previousLineIndex; indexLine < lines.length; indexLine++) {
                ...
                for (int indexRec = previousKeyIndex; indexRec < configStruct.recordsetStructureList.size(); indexRec++) {
                    ...
                    if (recordSS.fieldSeparator != '') {
                        ...
                        if (lineValues.size() <= recordSS.keyFieldIndex)
                            continue;
                        ...
                        if (!configStruct.keyFieldRequired || tempKeyLine == recordSS.keyFieldValue) {
                            ...
                            recordSet = handleRecordSet(xml, recordSet, previousRecordAddedIndex, recordSS, recordsetMap);

                            previousRecordAddedIndex = recordSS.index;
                            record = new Node(recordSet, recordSS.name);

                            int currRsIndex = configStruct.ignoreRecordSetName ? 0 : xml.children().size() - 1;
                            String rsKey = currRsIndex + "_" + recordSS.index;
                            Integer rsCount = recordsetMap.getOrDefault(rsKey, 0);
                            rsCount++;
                            recordsetMap.put(rsKey, rsCount);
    ...
    private Node handleRecordSet(Node xml, Node lastRecordSet, Integer previousRSSIndex,
                                 RecordSetStructure recordSS, HashMap<String, Integer> recordsetMap){
        if (!configStruct.ignoreRecordSetName) {
            if (lastRecordSet == null) {
                return createRecordSet(xml);
            }
            else {
                Integer rssOccurInt = recordSS.occur != '*' ? recordSS.occur as Integer : 0;
                Integer currRsIndex = xml.children().size() - 1;
                String rsKey = currRsIndex + '_' + recordSS.index;
                Integer rsCounter = recordsetMap.getOrDefault(rsKey, 0);

                //valid for all options
                if (rssOccurInt != 0 && rsCounter == rssOccurInt){
                    return createRecordSet(xml);
                }

                if (configStruct.recordsetStructureOrder == RSSOrder.VARIABLE) {
                    //all orders are supported, only create RS by max occurrence - generic case - not needed???
                }
                else if (configStruct.recordsetStructureOrder == RSSOrder.ASCENDING) {
                    //if order changes, create new RS or max occurrence
                    if (recordSS.index < previousRSSIndex){
                        return createRecordSet(xml);
                    }
                }
                else if (configStruct.recordsetStructureOrder == RSSOrder.DESCENDING) {
                    //TODO better understand expected behaviour
                    if (recordSS.index < previousRSSIndex){
                        return createRecordSet(xml);
                    }
                }
    ...
    private Node createRecordSet(Node xml){
        Node recordSet;
...
class Config {
    ...
    private def extractValues(String line, String fieldSeparator){
        ...
        int delimiterLength = fieldSeparator.size();

        for (int i = 0; i < line.length(); i++) {
            char currentChar = line.charAt(i);
            // Toggle the inEnclosure state if we encounter the enclosure character
            if (enclosureConversion && currentChar == enclosureSign.charAt(0)) {
                inEnclosure = !inEnclosure;
            }
            // Check if we are at the start of the delimiter
            if (!inEnclosure && i + delimiterLength <= line.length() && line.substring(i, i + delimiterLength).equals(fieldSeparator)) {
                result.add(currentSegment.toString().trim());
                currentSegment = currentSegment.delete(0, currentSegment.length()); // Reset the current segment
                i += delimiterLength - 1; // Move the index forward by the length of the delimiter
            } else {
...
class ConfigStruct extends Config {
    ...
    String recordsetStructureOrder;
    ...
    ConfigStruct(Message message, HashMap props) {
        ...
        def recordsetStructureOrderTemp = props.get("xml.recordsetStructureOrder");
        recordsetStructureOrder = recordsetStructureOrderTemp != null ? recordsetStructureOrderTemp as String : RSSOrder.ASCENDING;
...
class RSSOrder {
    ...
    final static String VARIABLE = 'var';
    final static String ASCENDING = 'asc';
    final static String DESCENDING = 'dsc';
