...
/**
* SetSearchValuesIDocCOND_A
* Set IDoc fields to header fields and log entries for search by IDoc number in message monitor.
* 'Material Number (18 Characters) (MATNR)', 'Material Number (MATNR_LONG)' and 'Account Number of Vendor or Creditor (LIFNR)' are supported.
...
def Message processData(Message message) {
	if (message.getBodySize() > 0) {
		...
		String matNr = root.IDOC.E1KOMG.MATNR[0].text() // COND_A0x
		String lifNr = root.IDOC.E1KOMG.LIFNR[0].text() // COND_A0x	
		String matNrLong = root.IDOC.E1KOMG.MATNR_LONG[0].text() // COND_A0x
