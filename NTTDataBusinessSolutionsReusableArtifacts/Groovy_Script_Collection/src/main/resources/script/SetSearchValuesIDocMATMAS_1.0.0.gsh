...
/**
* SetSearchValuesIDocMATMAS
* Set IDoc fields to header fields and log entries for search by IDoc number in message monitor.
* 'Material number (18 Characters) (MATNR)', 'Material number long (MATNR_LONG)', 'Material Description (MAKTX)' are supported.
...
def Message processData(Message message) {
	if (message.getBodySize() > 0) {
		...
		String matNr = root.IDOC.E1MARAM.MATNR[0].text() // MATMAS
