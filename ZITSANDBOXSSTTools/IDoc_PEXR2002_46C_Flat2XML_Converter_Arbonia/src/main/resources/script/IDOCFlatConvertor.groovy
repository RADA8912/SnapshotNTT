...
class TransformFlat2XML extends Transform {
    ...
    static String convertIDOC(String[] lines, XMLSchemaNode xmlSchema){
        Node xml = matchAndCreate(lines, xmlSchema);
        String resPayload = xmlToStringClean(xml);
        return resPayload;
    }

    static private Node matchAndCreate(String[] lines, XMLSchemaNode xmlSchema){
        ...
        for (int iLine = 0; iLine < lines.size(); iLine++){
            ...
            if (line.startsWith("EDI_DC40")){
                ...
                createNodes(EDI_DC40, offset, line, ediNode);
            }
            else {
                Integer offset = 0;
                //30 -> segmentName
                ...
                if (lineNumberParent == 0) {
                    handleSegments(iSegment, xsIDOC, idocNode, line, lineLabel, offset, lineNumber, auxNodeArray);
                }
                else {
                    ...
                    handleSegments(iSegment, segmentTemp, segmentNode, line, lineLabel, offset, lineNumber, auxNodeArray);
                }
            }
        }
    ...
    static private void handleSegments(int iSegment, XMLSchemaNode xsParent, Node xmlParent, String line, String label,
        ...
        for (iSegment; iSegment < xsParent.subNodes.size(); iSegment++) {
            ...
            if (identifyNode(segment, label)) {
                ...
                createNodes(segment, offset, line, newNode);
                break;
            }
        }
        if (!found) {
    ...
    private static void createNodes(XMLSchemaNode segment, int offset, String line, Node newNode){
