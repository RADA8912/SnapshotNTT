...
def Message browse(Message message) {
    ...
    String urlDown = srvUrl.minus(mainSrv)+"/zit/sgr/filesystemdownload"
...
def Message download(Message message) {
    ...
    mapHeaders = ['Content-Transfer-Encoding':'base64', 'Content-Disposition':'attachment; filename=${file.getName()}']
    return message
}