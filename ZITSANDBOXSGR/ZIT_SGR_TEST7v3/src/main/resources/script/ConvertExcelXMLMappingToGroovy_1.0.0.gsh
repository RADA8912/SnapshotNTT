		...
		body.eachLine { line, count ->
			...
			if (lineLength > 0) {
				...
				if (indexOfSeparator != -1) {
					// Make max columns
					String lineCompletely = line + (plainColumnSeparator * 10)
					// Split line
					def columList = lineCompletely.split(plainColumnSeparator, -1)
					if (count == 0) {
						// Read header lines
						...
						// Get Variable
							...
							if (!sourceNode) {
								...
								sourceNode = sourceNode.substring(1, sourceLength) + ".text()"
