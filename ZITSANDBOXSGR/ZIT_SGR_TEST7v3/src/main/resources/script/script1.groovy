...
def Message processData(Message message) {
	...
	if (message.getBodySize() > 0) {
		...
		// Variables used in mapping
		String my_var1 = '' 

		// TODO: Create sub segment formulas
		// TODO: Create complex mapping formulas

		// Map nodes to target structure
		// Materials -> XPath: /Materials
		// Map Rule: constant
		xml.Materials {
			// Material -> XPath: /Materials/Material
			// Map Rule: constant
			Material {
				// Material Number -> XPath: /Materials/Material/MaterialNumber
				// Map Rule: 1:1
				'MaterialNumber' ns0:MATMAS06.IDOC.E1MARAM.MATNR.text()
				// Material Name -> XPath: /Materials/Material/MaterialName
				// Map Rule: 1:1
				'MaterialName' ns0:MATMAS06.IDOC.E1MARAM.E1MAKTM.MAKTX.text().toLowerCase()
				// Message ID -> XPath: /Materials/Material/MessageID
				// Map Rule: computed
				'MessageID' message.getProperty('SAP_MessageProcessingLogID')
				// Sender -> XPath: /Materials/Material/Sender
				// Map Rule: computed
				'Sender' message.getHeader('SAP_Sender', String)
				// Receiver -> XPath: /Materials/Material/Receiver
				// Map Rule: computed
				'Receiver' message.getHeader('SAP_Receiver', String)
				// Date -> XPath: /Materials/Material/Date
				// Map Rule: computed
				'Date' setCurrentDate(timeZone, "yyyyMMdd'T'HHmmss")
				// Time -> XPath: /Materials/Material/Time
				// Map Rule: computed
				'Time' setCurrentDate(timeZone, "HHmmss")
				// File name -> XPath: /Materials/Material/FileName
				// Map Rule: Take value from Property
				'FileName' getExchangeProperty(message, 'FileName', false)
				// Transport -> XPath: /Materials/Material/Transport
				// Map Rule: "constant ""Truck"""
				'Transport' 'Truck'
				// Unit of measure -> XPath: /Materials/Material/UnitOfMeasure
				// Map Rule: 1:1
				'UnitOfMeasure' mapWithDefault(ns0:MATMAS06.IDOC.E1MARAM.MEINS.text(), 'KGM')
				// Gross weight -> XPath: /Materials/Material/GrossWeight
				// Map Rule: 1:1
				'GrossWeight' removeAlgebraicSign(ns0:MATMAS06.IDOC.E1MARAM.BRGEW.text())
				// Net weight -> XPath: /Materials/Material/NetWeight
				// Map Rule: computed
				'NetWeight' replaceSpecialCharacters(ns0:MATMAS06.IDOC.E1MARAM.NTGEW.text())
				// Creation date 2 -> XPath: /Materials/Material/CreationDate2
				// Map Rule: computed
				'CreationDate2' setDefaultAsCurrentDate(timeZone, MATMAS06.IDOC.EDI_DC40.CREDAT.text(), "yyyyMMdd'T'HHmmss")
				// Creation date 3 -> XPath: /Materials/Material/CreationDate3
				// Map Rule: computed
				'CreationDate3' '' // TODO: Create mapping formula.
				// -> XPath: /Materials/*my_var1
				// Map Rule: variable
				my_var1 = '123'
				// Creation type 4 -> XPath: /Materials/Material/CreationType
				// Map Rule: computed
				'CreationType' my_var1
			}
		}
...
private def String fixedValues(String value, def mapTable, String behaviour, String defaultValue) {
	String result = ''
	String behaviourStr = behaviour.toLowerCase().replaceAll(' ','')
	// Check input values
	...
	} else if (!('defaultvalue'.equals(behaviourStr) || 'usekey'.equals(behaviourStr) || 'throwexception'.equals(behaviourStr))) {
...
/**
* Creates a yyyyMMdd date or time by using an EDIFACT-F2379-conform qualifier.
*
* @param locale Locale
* @param dateValue Date values
* @param dateFormat EDIFACT Date format ('2', '3', '101', '102', '201, '202', '203', '204', '401', '402', '718')
* @param mode Mode ('date', 'dateFrom', 'dateTo', 'time')
* @return formatted date values (yyyyMMdd) if a date-mode is set or time if time-mode is set.
*/
private def String getDate(Locale locale, String dateValue, String dateFormat, String mode) {
	String result = ''
	// Check locale
	if (!locale) {
		throw new Exception("Custom Function getDate: cannot get locale.")
	}
	// Format date
	if (dateValue) {
		if (!"date".equalsIgnoreCase(mode) && !"dateto".equalsIgnoreCase(mode) && !"datefrom".equalsIgnoreCase(mode) && !"time".equalsIgnoreCase(mode)) {
			throw new Exception("Custom Function getDate: unvalid mode '" + mode + "'. Please use Custom Function constantDate, constantDateFrom, constantDateTo or constantTime.")
		}
		try {
			String dateValueStr = dateValue.trim()
			int dateLength = dateValueStr.length()
			int dtmFormat = Integer.parseInt(dateFormat)
			boolean unexpectedLength = false
			boolean modeIsDateTo = "dateto".equalsIgnoreCase(mode)
			boolean modeIsTime = "time".equalsIgnoreCase(mode)
			String date = null
			if (dateValue == null || dateValue.length() == 0 || dateValue.replaceAll("^0*", "").length() == 0) {
				result = ''
			} else {
				try {
					switch (dtmFormat) {
						case 2: // DDMMYY
							if (dateLength != 6) {
								...
								date = "20" + dateValueStr.substring(4, 6) + dateValueStr.substring(2, 4) + dateValueStr.substring(0, 2)
							}
							break
						case 3: // MMDDYY
							if (dateLength != 6) {
								...
								date = "20" + dateValueStr.substring(4, 6) + dateValueStr.substring(0, 2) + dateValueStr.substring(2, 4)
							}
							break
						case 101: // YYMMDD
							if (dateLength != 6) {
						...
						case 201: // YYMMDDHHMM
							if (dateLength != 10) {
								...
								date = "20" + dateValueStr.substring(0, 6)
							} else if (modeIsTime) {
								date = dateValueStr.substring(6, 10)
							}
							break
						case 202: // YYMMDDHHMMSS
							if (dateLength != 12) {
								...
								date = dateValueStr.substring(6, 12)
							}
							break
						case 203: // CCYYMMDDHHMM
							if (dateLength != 12) {
								...
								date = dateValueStr.substring(0, 8)
							} else if (modeIsTime) {
								date = dateValueStr.substring(8, 12)
							}
							break
						case 204: // CCYYMMDDHHMMSS
							if (dateLength != 14) {
								...
								date = dateValueStr.substring(8, 14)
							}
							break
						case 401: // HHMM
							if (dateLength != 4) {
						...
						case 718: // CCYYMMDD-CCYYMMDD with and without hyphen
							if (dateLength == 17) {
								dateValueStr = dateValueStr.replaceAll("-", "")
								dateLength = dateValueStr.length()
							}
							if (dateLength != 16) {
								unexpectedLength = true
							} else if (modeIsDateTo) {
								date = dateValueStr.substring(8)
							} else if (!modeIsTime) {
								date = dateValueStr.substring(0, 8)
							}
							break
						default:
							throw new Exception("Custom Function getDate: Unexpected date time EDIFACT qualifier '" + dateFormat + "'.")
					}
					if (unexpectedLength) {
						throw new Exception("Custom Function getDate: Length of date string '" + date + "' does not fit date format '" + dateFormat + "'.")
					}
					// Format and set date
					if (date == null) {
						date = ''
						...
						try {
							...
							throw new Exception("Custom Function getDate: can not parse date string '" + date + "' of format '" + dateFormat + "'.")
						}
					}
					result = date
				} catch (RuntimeException ex) {
					throw new Exception("Custom Function getDate: dateValue '" + dateValue + "', dateFormat '" + dateFormat + "' causes '" + ex + "'.")
				}
			}
		} catch (Exception ex) {
			throw new Exception("Custom Function getDate: " + ex, ex)
		}
	}
	return result
}
...
private def String setFileName(Message message, def timeZone, String messageType, String messageVersion, String messageRelease, boolean addTimeStamp, String dateFormat, String documentNumber, boolean addMessageId, boolean addCorrelationId, String nameSeparator, String fileType) {
	...
	def dateNew
	String timeStamp = ''
	String dateFormatNew = dateFormat
	if (messageType && fileType && addMessageId != null && addCorrelationId != null && addTimeStamp != null) {
		// Set nameSeparator
		String nameSeparatorNew = nameSeparator
		if (!nameSeparatorNew) {
			nameSeparatorNew = NAME_SEPARATOR
		}
		// Add EDI message release
		if (messageVersion) {
			fileName += nameSeparatorNew + messageVersion
		}
		// Add EDI message release
		if (messageRelease) {
			fileName += nameSeparatorNew + messageRelease
		}
		// Add time stamp
		if (addTimeStamp) {
			try {
				if (!dateFormatNew) {
					dateFormatNew = DATE_FORMAT
				}
				// Set simple date format
				SimpleDateFormat sdf = new SimpleDateFormat(dateFormatNew)
				sdf.setTimeZone(timeZone)
				// Create new date
				date = new Date()
				// Format date
			...
			fileName += nameSeparatorNew + timeStamp
		}
		// Add document number
		if (documentNumber) {
			fileName += nameSeparatorNew + documentNumber
		}
		// Add message ID
		if (addMessageId) {
			...
			fileName += nameSeparatorNew + messageId
		}
		// Add correlation ID
		if (addCorrelationId) {
			...
			fileName += nameSeparatorNew + correlationId
		}
		// Add file type
