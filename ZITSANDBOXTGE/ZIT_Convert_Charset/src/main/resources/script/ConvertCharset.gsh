...
* - ConvertEncoding.SetBomTargetFile = True, sets the ByteOrderMark
...
def Message processData(Message message) 
{
    ...
    replacementChar = getExchangeProperty(message, "ReplacementChar", false)
    ...
    sourceEncoding = getExchangeProperty(message, "SourceEncoding", true);
    messageLog.addCustomHeaderProperty("SourceEncoding", sourceEncoding);

	targetEncoding = getExchangeProperty(message, "TargetEncoding", true);
    messageLog.addCustomHeaderProperty("TargetEncoding", targetEncoding);
   
   
    try
    {
        def boolConv = getExchangeProperty(message, "SetBomTargetFile", false);
        ...
        setBomUtf8 = Boolean.parseBoolean(boolConv);
  	    messageLog.addCustomHeaderProperty("setBomTargetFile", setBomUtf8.toString());
    }
    catch(Exception e)
    {
    ...
    byte[]  fileBytes = bodyString.getBytes(sourceEncoding);
    ...
    String ansiContent = new String(fileBytes, targetEncoding)
    ...
    */
    def hexDumpOutput = new StringBuilder();
    def targetBytes = ansiContent.getBytes(targetEncoding);
    for (byte b in targetBytes) 
    {
        hexDumpOutput.append(String.format("%02X ", b))
    }
    messageLog.addCustomHeaderProperty("Outgoing", hexDumpOutput.toString());
    message.setBody(ansiContent)
	return message;
}
