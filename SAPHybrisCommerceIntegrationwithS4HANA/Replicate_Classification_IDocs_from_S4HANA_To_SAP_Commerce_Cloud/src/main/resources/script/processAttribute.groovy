...
def Message processData(Message message) {
	...
	def chrmas = message.getBody(java.lang.String) as String;
	def clsmas = message.getProperty("CLS_XML") as java.lang.String;
	def cls_root = new XmlSlurper().parseText(clsmas);
	...
	def batchAttributeValueList = chr_root.IDOC.E1CABNM.E1CAWNM.collect{it->it.ATWRT.text()};
	...
	message.setBody(XmlUtil.serialize('<CLSMAS>'+(attributeValueBatchParts.toString()=='<batchParts></batchParts>'?'':attributeValueBatchParts) +attributeBatchParts+assignmentBatchParts+'</CLSMAS>'));
