...
@Field String IFLOW_NAME = 'Greece Send Invoice'
...
@Field String PROPERTY_ENABLE_MPL_LOGGING 		= 'ENABLE_PAYLOAD_LOGGING'
@Field String PROPERTY_ENABLE_FILE_LOGGING 		= 'ENABLE_PAYLOAD_LOGGING'
...
//    Logger log = LoggerFactory.getLogger(this.getClass());
    ...
    try {
        ...
        processHeadersAndProperties(    IFLOW_NAME+"_log_{COUNTER}_info",           message);
        processBody(                    IFLOW_NAME+"_log_{COUNTER}_payload",   message);
        processAttachmentBody(          IFLOW_NAME+'_log_{COUNTER}_attachment_',   message);
...
def Message log_010(Message message) {
    ...
    try {
        increaseInternalHeaders(message);
        increaseInternalProperties(message);
        processHeadersAndProperties(    IFLOW_NAME+'_010_{COUNTER}_info',           message);
        processBody(                    IFLOW_NAME+'_010_{COUNTER}_payload',   message);
        processAttachmentBody(          IFLOW_NAME+'_010_{COUNTER}_attachment_',   message);
...
def Message log_020(Message message) {
    ...
    try {
        increaseInternalHeaders(message);
        increaseInternalProperties(message);
        processHeadersAndProperties(    IFLOW_NAME+"_020_{COUNTER}_info",           message);
        processBody(                    IFLOW_NAME+"_020_{COUNTER}_payload",   message);
        processAttachmentBody(          IFLOW_NAME+'_020_{COUNTER}_attachment_',   message);
    } catch (Exception ex) {
        log.error("processData error",ex);
    }
    return message;
...
def Message log_030(Message message) {
    ...
    try {
        ...
        processAttachmentBody(          IFLOW_NAME+'_030_{COUNTER}_attachment_',   message);
    } catch (Exception ex) {
        log.error("processData error",ex);
    }
    return message;
