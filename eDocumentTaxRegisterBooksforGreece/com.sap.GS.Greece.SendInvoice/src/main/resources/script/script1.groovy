...
def Message processData(Message message) {
   ...
   //Properties 
       ...
       def vatIDIssuer   = map.get("AFM");
       def vatIDCounter = map.get("AFM_Counter")
       def invoiceType = map.get("Invoice_Type")
       def variationType = map.get("Variation_Type")
	   ...
	   if( (invoiceType >= "13.1" && invoiceType <= "16.1" ) ||
	       (variationType == "1"   || variationType == "3" ) )
	   {
	       vatID = vatIDCounter;
	   }else{
	       vatID = vatIDIssuer;
