...
*/
	...
	boolean methodToLowerCase = false
	boolean methodToNumber = false
	boolean methodToUpperCase = false
...
*/
	...
	String plainColumnSeparator = getExchangeProperty(message, "ConvertMap.PlainColumnSeparator", false)
			...
			if (lineLength > 0) {
				...
				if (indexOfSeparator != -1) {
					...
					if (count == 0) {
						...
						// Remove leading and end double quotation mark and two double quotation mark in string
						if (sourceNode.startsWith('"') && sourceNode.endsWith('"')) {
							sourceNode = sourceNode.substring(1, sourceNode.length() - 1)
							sourceNode = sourceNode.replace('\"\"', '"')
						}
						// Remove double // if wrong in put in Excel
						sourceNode = sourceNode.replace('//', '/')
						...
						// Set Variable
						} else if (sourceNode.startsWith('setVariable(') && bracketCount == 1) {
							sourceNode = setSimpleVariable(sourceNode)
							sourceType = 'constant'
							message.setProperty('ConvertMap.LastLineSourceFormula','setVariable')
						// Get Variable
						} else if (sourceNode.startsWith('getVariable(') && bracketCount == 1) {
							sourceNode = getSimpleVariable(sourceNode)
							sourceType = 'formula'
							message.setProperty('ConvertMap.LastLineSourceFormula','getVariable')
						} else {
...
private String getSimpleSetCurrentDate(String sourceNode) {
	...
	if (sourceNode.indexOf(',') > -1) {
		...
		dateFormat = sourceNode.substring(13, sourceNode.length()-1)
...
private String setSimpleVariable(String sourceNode) {
	if (sourceNode.startsWith('setVariable')) {
		sourceNode = sourceNode.replaceAll('\\)','')
		// Split into 2 parts
		def tmpList = []
		tmpList = sourceNode.split(',', -1)
		sourceNode = tmpList[1]
	}
	return sourceNode
}
...
private String getSimpleVariable(String sourceNode) {
	if (sourceNode.startsWith('getVariable')) {
		sourceNode = sourceNode.substring(13, sourceNode.length() - 2)
	} else {
		sourceNode = sourceNode.substring(1, sourceNode.length())
	}
	return sourceNode
